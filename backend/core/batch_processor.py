...
from backend.core.sparse_hamiltonian import SparseHamiltonianEngine
...
class BatchProgress:
    ...
    def progress_pct(self) -> float:
        return (
            (self.completed + self.failed) / self.total * 100 if self.total > 0 else 0
        )
...
class BatchProcessor:
    ...
    def __init__(
        self, engine_type: str = "boosted", max_concurrency: int = 4, **engine_kwargs
    ):
        """
        ...
        logger.info(
            f"BatchProcessor initialized: engine={engine_type}, concurrency={max_concurrency}"
        )
    ...
    def _create_engine(self):
        ...
        if self.engine_type == "boosted":
            ...
            return QuantumResistantZKAEDI(
                **{k: v for k, v in self.engine_kwargs.items() if k in ["alpha", "eta"]}
            )
    ...
    async def _scan_single(
        self, network_id: str, network_graph: Dict[str, List[str]], max_iterations: int
    ) -> Dict:
        ...
        try:
            ...
            if self.engine_type == "boosted":
                result = await engine.solve_vuln_detection_boosted(
                    network_graph=network_graph, max_iterations=max_iterations
                )
            elif self.engine_type == "ultra":
    ...
    async def process_batch(
        self, networks: List[Dict[str, Any]], max_iterations: int = 50000
    ) -> BatchResult:
        """
        ...
        results = await asyncio.gather(*[scan_with_semaphore(net) for net in networks])
        ...
        logger.info(
            f"Batch {batch_id} complete: {len(successful_results)}/{total} successful in {total_time:.2f}s"
        )
...
async def example_batch_processing():
    ...
    # Create processor
    processor = BatchProcessor(engine_type="boosted", max_concurrency=3)
    ...
    # Process batch
    result = await processor.process_batch(networks=networks, max_iterations=5000)
